    ...
    <div style={{ padding: '40px 20px', display: 'flex', flexDirection: 'column', alignItems: 'center', minHeight: '100vh', justifyContent: 'center' }}>
      ...
      <div className="login-form" style={{ width: '100%', maxWidth: 340 }}>
        ...
        {modo !== 'reset' && (
          <div style={inputWrap}>
            ...
            {modo === 'registro' && !errores.password && (
              <div style={{ fontSize: 11, color: 'var(--c-muted)', marginTop: 4, marginLeft: 4 }}>
        ...
        {blockedUntil && Date.now() < blockedUntil && (
          <div style={{
            background: 'rgba(239,68,68,0.1)', color: '#EF4444', fontSize: 12, padding: '10px 14px',
            borderRadius: 10, marginBottom: 12, textAlign: 'center', fontWeight: 600, border: '1px solid rgba(239,68,68,0.3)',
          }}>
        ...
        <button onClick={handleSubmit} disabled={loading || !!(blockedUntil && Date.now() < blockedUntil)} style={{
          ...
          background: (loading || (blockedUntil && Date.now() < blockedUntil)) ? 'var(--c-muted)' : 'var(--c-primary)', color: '#fff',
          fontSize: 16, fontWeight: 800, cursor: (loading || (blockedUntil && Date.now() < blockedUntil)) ? 'default' : 'pointer',
          ...
          <>
            ...
            <button onClick={async () => {
              ...
              try {
                ...
                // En web, signInWithOAuth redirige automáticamente
              } catch (err) {
